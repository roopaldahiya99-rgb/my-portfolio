...
function initMap() {
  ...
  // On mobile, override to the tuned mini-map view
  if (isMobile) {
...
function renderDiaryRight() {
  ...
  const fieldPanel = `
    <div class="diary-panel active" data-panel="field">
      <p class="diary-panel-intro">Lessons from dealwork — how to assess, what I believe, how to show up.</p>
          ...
          <div class="sources-section">
            ...
            ${cat.sources.map(src => `
              <div class="source-card">
                ...
                ${src.favourite ? `<a href="${src.favourite}" target="_blank" rel="noopener" class="sources-fav"><em>Favourite Read: ${src.favouriteName || 'Read'}</em></a>` : ''}
              </div>
            `).join('')}
          </div>
        `).join('')}
